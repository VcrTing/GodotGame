...
public partial class BulletPaoGu : Node2D, IBulletBase, IObj, IAttack
{
    
    [Export]
    public EnumHurts hurtType { get; set; } = EnumHurts.Pea;
    [Export]
    public string objName = BulletConstants.BulletPaoGuName;
    [Export]
    public float RotationSpeed = 0f; // 每秒旋转角度
    [Export]
    public float SpeedSubNumStay = 30f;
    [Export]
    public float SpeedSubNumFadeOut = 60f;
    [Export]
    public float DamageLowestRatio = 0.5f;
    [Export]
    public float DamageStayTime = 0.2f;
    [Export]
    public float DamageFadeTime = 0.7f;
    ...
    public override void _Ready()
    {
        // 给Scale加一点随机数加减
        float s = Scale.X;
        s += (GD.Randf() * 0.3f) - 0.15f; // -0.2到0.4之间
    ...
    bool IsInBigger = true;
    float __biggerTime = 0f;
    ...
    public override void _Process(double delta)
    {
        ...
        else
        {
            ...
            {
                ViewTool.View3In1(this, minScale, maxScale);
                __biggerTime += (float)delta;
                float x = Scale.X;
                x += (__biggerTime / 300);
                Scale = new Vector2(x, x);
                if (__biggerTime >= 2f) { __biggerTime = 0f; IsInBigger = false; }
            }
            else
            {
                ViewTool.View3In1(this, minScale, maxScale);
        ...
        if (__at > AttackSpeed)
        {
            ...
            if (__at >= (AttackSpeed + attackStayTime))
            {
    ...
    void ChangeAttackArea(bool active)
    {
        try
        {
            if (_area2D == null) _area2D = GetNodeOrNull<Area2D>(NameConstants.AttackArea);
            if (_area2D != null)
            {
                _area2D.Visible = active;
                _area2D.SetDeferred("monitoring", active);
            }
        }
        catch { }
    }
